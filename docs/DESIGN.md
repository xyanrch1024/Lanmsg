...
┌──────────────▼──────────────────────────────────────▼────────────────────┐
│ 发现层 (src/discovery)                                                     │
│  PeerDiscovery:UDP 上线广播 + 新设备应答 + 下线(bye),输出 Peer 列表      │
└──────────────┬──────────────────────────────────────┬────────────────────┘
               │ UDP 24260                             │ TCP 24261
┌──────────────▼─────────────────────┐  ┌─────────────▼─────────────────────┐
│ 局域网 (LAN)                        │  │ 对端设备(同一套程序)                │
...
- 启动时广播一次 `255.255.255.255`,并加入组播组 `239.255.77.77` 同步发送一份;同时对已知对端**单播**一份(WSL2/NAT 隔离广播域时的补救);之后以 **10 秒间隔周期保活广播**(自愈:对端重启/断网重连后,即使没有 `bye`、对方仍保留旧条目且对新设备之外的 hello 不回包,双方也能在一个周期内重新互相发现)。
  - 组播的必要性:Windows 下多个 socket 共享 UDP 端口时,广播/单播只投递给其中一个 socket,同机多实例会互不可见;组播会投递给所有已加入的成员,保证同机多实例能互相发现。
  - 本机 IP 变化(换 Wi-Fi/VPN/WSL2 地址变动)时自动**重新广播一次**,保证新网段仍能发现;改名后也会重发。
- 收到合法数据包后,仅对**首次出现的新设备**回复一份自己的信息(防止两客户端互相回包形成回显风暴),其余情况不回复。
- 数据包带 `cmd` 字段:**`hello`**(上线/应答)与 **`bye`**(下线)。收到 `bye` 立即移除该设备(不回包,避免"复活");无超时清理——对端崩溃(未发 bye)会残留列表,直到周期保活/重启刷新。

数据包为紧凑 JSON:

```json
{ "magic": "QLMSG", "ver": 1, "cmd": "hello", "id": "<12hex>", "name": "昵称",
...
5. **优先保留就绪会话**:若两条中一条已 `ready`、另一条仍在连接,无条件保留就绪者,禁止用连不上的会话顶掉可用会话(否则单向连接被防火墙丢弃时该端会永久卡死)。仅在两者就绪状态相同时才比较握手键。
6. **出站连接超时**:Outgoing 会话 `connected` 后 12 秒仍未就绪则 `abort()` 并注销,避免被静默丢弃的 SYN 长期占用会话槽;下次操作会惰性重连。

> WSL2/NAT 边界提示:Windows ↔ WSL2 之间 TCP 可能单向不可达(常见原因:Windows 防火墙未放行入站,或 WSL2 NAT/镜像模式差异)。发现(UDP)正常但 TCP 连不上时,优先排查 Windows 入站规则(24260/24261),或为 WSL2 开启 `networkingMode=mirrored`。
...
- **代理软件冲突**:全局代理软件(Clash、V2rayN 等)会通过环境变量(`ALL_PROXY`/`socks_proxy`)注入 SOCKS 代理,Qt 默认对其所有 socket 生效;对 UDP 发现 socket 会导致 `bind()` 失败、`joinMulticastGroup` 报 "not in BoundState" / "Connection not allowed by SOCKSv5 server"。应用在 `main()` 中强制 `QNetworkProxy::setApplicationProxy(NoProxy)` 全局禁用代理(QLanMsg 仅走局域网,无上网功能)。
...
- **发现**:A 启动广播 `hello`、B 启动广播 `hello`,互为新设备时立即互答,即刻发现对方;退出时互发 `bye` 即时移除。
