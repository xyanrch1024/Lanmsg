...
- **仅在启动时广播一次** `255.255.255.255`,并加入组播组 `239.255.77.77` 同步发送一份;同时对已知对端**单播**一份(WSL2/NAT 隔离广播域时的补救)。无周期保活广播。
...
- 数据包带 `cmd` 字段:**`hello`**(上线/应答)与 **`bye`**(下线)。收到 `bye` 立即移除该设备(不回包,避免"复活");无超时清理——对端崩溃(未发 bye)会残留列表,直到重启/重广播刷新。
